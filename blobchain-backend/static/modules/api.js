...
import { getRequests, getRestaurants, remRequest } from "./firebase.js";
...
const apiExistingUser = async (email, password) => {
...
const apiGetReservations = async (userAddr, isVisited) => {
  const slots = {};
  const rawSlots = await getUserSlots(userAddr, isVisited);
  for (const slot of rawSlots) {
    const date = new Date(Number.parseInt(slot.token.dateTime));
    const dateString = date.toDateString();
    if (!slots[dateString]) slots[dateString] = [];
